...
import (
	...
	"github.com/0x587/guardeye/test-client/conn/downstream/ros/implcli/rosmsg/walk"
)

func (i *impl) getInterfaceMsgDefine(typeUrl string) (*ros.MsgDefine, error) {
	...
	block := &ros.BlockDefine{
		Fields: lo.Map(msg.Fields, func(f *walk.FieldDefine, _ int) *ros.FieldDefine {
			return &ros.FieldDefine{
				Name:        f.Name,
				Type:        f.Type.Name,
...
func (i *impl) getInterfaceSrvDefine(typeUrl string) (*ros.SrvDefine, error) {
	typePaths := strings.Split(typeUrl, "/")
	packageName, typeName := typePaths[0], typePaths[len(typePaths)-1]
	interfaceDefine, err := i.rosInterfaceShow(typeUrl)
	if err != nil {
...
func (i *impl) processMsg(msg *ros.BlockDefine, packageName string) error {
	for _, f := range msg.Fields {
		...
		m := walk.Walk(tree)[0]
		block := &ros.BlockDefine{
			Fields: lo.Map(m.Fields, func(f *walk.FieldDefine, _ int) *ros.FieldDefine {
				return &ros.FieldDefine{
					Name:        f.Name,
					Type:        f.Type.Name,
					IsArray:     f.Type.IsArray,
