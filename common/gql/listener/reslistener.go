...
func parseDuration(n antlr.TerminalNode, u parser.ITimeUnitContext) (time.Duration, error) {
	...
	switch u.GetText() {
	default:
		return 0, errors.Errorf("unknown time unit %s", u.GetText())
	case "m":
		return time.Duration(parseInt) * time.Minute, nil
	case "h":
		return time.Duration(parseInt) * time.Hour, nil
	case "d":
		return time.Duration(parseInt) * 24 * time.Hour, nil
	}
}
...
func (l *ResListener) EnterResultColumn(ctx *parser.ResultColumnContext) {
	...
	if ctx.WITH_() != nil {
		re.WindowFunc = ctx.WindowFunction().GetText()
	}
	alias := ctx.ResultAlias()
	if alias != nil {
		re.Alias = alias.GetText()
	} else {
		re.Alias = ctx.ValueExpr().GetText()
	}
...
func (l *ResListener) EnterWindowSizeStmt(ctx *parser.WindowSizeStmtContext) {
	d, err := parseDuration(ctx.NUMERIC_LITERAL(), ctx.TimeUnit())
	if err == nil {
		l.qe.windowSize = &d
	}
}

//func wrapValueWithWindowFunc(v *ValueEntry, w parser.IWindowFunctionContext) *ValueEntry {
//	oldVf := v.Vf
//	switch w.GetText() {
//	default:
//		panic(errors.Errorf("unknown window function: %s", w.GetText()))
//	case "avg":
//		v.Vf = func(ij EnvInjector) (any, error) {
//			return oldVf(ij)
//		}
//	}
//	return v
